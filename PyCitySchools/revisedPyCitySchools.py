...
# 
# There are 15 total schools included in PyCity.  
# There are a total of 39,170 students, and the district total budget is about $25,650,000.  
# Eight of the 15 schools are charter schools.  All of the schools range in size of enrollment 
# from 427 students on the low end, all the way up to 4,719 students.  All but 3 of the shools 
# have enrollments greater than 1500 students.
# 
# Student Math and Reading scores were merged with data about the schools in order to do an 
# analysis of the results.  One takeaway is that the Charter Schools generally have 
# lower per student Budgets.  In addition, the highest performing schools, in terms of 
# percent of students passing both math and reading, were all Charter schools. 
# The bottom 5 performing schools were all District schools.  
# 
# Comparing bins of spending ranges to math and reading results showed that schools with the 
# lowest per student budgets had higher rates of passing math and reading.  Schools that spent more 
# per student actually had lower passing rates.  This seems counterintuitive.  One would think that 
# spending more would likely help at least some schools to raise results well enough to positively 
# affect the averages.  This may be partially explained by looking at the size of the schools. 
 
# Small and medium schools (less than 2000 students) had relatively higher passing rates and lower 
# per student budgets.  Passing rates for large schools were quite a bit lower, and these schools, 
# on average, have higher spending per student.  It seems that perhaps there is something about running 
# a large school that requires more money and does not provide the same high achievement.
#   
# ---
...
passing_math_reading_count = data_complete[
    (data_complete["math_score"] >= 70) & (data_complete["reading_score"] >= 70)].count()["student_name"]
...
per_school_summary = pd.DataFrame({"School Type":school_types, "Student Count":per_school_stud_count, 
                                   "Budget":per_school_budget, "Budget per Student":per_school_capita,
                                   "Average Math Score":math_avgs, "Average Reading Score":reading_avgs, 
                                   "Percent Passing Math":per_school_math_percent,
                                   "Percent Passing Reading":per_school_reading_percent, 
                                   "Percent Passing Both":per_school_both_percent})
# Display the DataFrame
per_school_summary
...
spending_math_scores = organized_df.groupby(["Spending Ranges(Per Student)"], 
                                            observed=False)["Average Math Score"].mean()
spending_reading_scores = organized_df.groupby(["Spending Ranges(Per Student)"], 
                                               observed=False)["Average Reading Score"].mean()
spending_passing_math = organized_df.groupby(["Spending Ranges(Per Student)"],
                                             observed=False)["Percent Passing Math"].mean()
spending_passing_reading = organized_df.groupby(["Spending Ranges(Per Student)"],
                                                observed=False)["Percent Passing Reading"].mean()
overall_passing_spending = organized_df.groupby(["Spending Ranges(Per Student)"],
                                                observed=False)["Percent Passing Both"].mean()
